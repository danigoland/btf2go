...
func TestRunTier4NegativeDuration(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "t.md")
	body := `---
...
`
	...
	got := RunTier4(path)
	if got[0].Status != StatusFail || !strings.Contains(got[0].Detail, "before start") {
		t.Fatalf("expected FAIL with negative-duration detail, got %+v", got)
	}
}
